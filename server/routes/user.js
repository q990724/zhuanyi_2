...
var fs = require('fs');
//登录接口
router.get("/login/:number&:upwd", function (req, res) {
  ...
  if (phone.indexOf("@") != -1) {
    console.log("邮箱登录",phone);
    sql += `email='${phone}' and upwd = ${upwd}`;
  } else if (phone.length == 11) {
    console.log("手机号登录",phone);
    sql += `phone='${phone}' and upwd = ${upwd}`;
  } else {
    console.log("用户名登录",phone);
    sql += `uname='${phone}' and upwd = ${upwd}`;
  }
  console.log(sql);
